...
describe('SettingsView — selling-home toggle (P7, B7)', () => {
  ...
  it('shows rent-paid for every adult; gates only the house link and co-owners', async () => {
    ...
    expect(ashbySection.find('input#rent-paid').exists()).toBe(true)
  ...
  it('saves the rent-paid figure with the person', async () => {
    ...
    await flush()
    const simon = personSection(wrapper)
    expect((simon.find('input#rent-paid').element as HTMLInputElement).value).toBe('600.00')
    await simon.find('input#rent-paid').setValue(650)
    await simon.trigger('focusout')
    await flush()
