...
class BusRouteNode(DerivedNode[dict]):
    ...
    def compute(self, location: Attempt, walk_check: Attempt,
                transit: Attempt) -> Attempt[dict]:
...
class BusLegAugmentNode(DerivedNode[dict]):
    ...
    def compute(self, transit: Attempt, walk_check: Attempt,
                route: Attempt, fare: Attempt) -> Attempt[dict]:
        if transit.succeeded:
            return transit
        if not walk_check.succeeded or not route.succeeded:
            return self._impossible({"walk_leg_check_node": walk_check,
