...
# Add project root to path so we can import houses modules
...
MAX_KM = 200  # ~3 hour commute radius


def _build_full_address(row: dict[str, Any]) -> str:
    """Build a full, geocodable address from a GIAS row."""
    ...
    parts = [name, street, locality, town, county, postcode]
    return ", ".join(p for p in parts if p)


def _existing_coords(row: dict[str, Any]) -> GeoPoint | None:
    """Return the existing CSV coordinates, or None if missing."""
...
def _near_london(row: dict[str, Any]) -> bool:
    """Check whether a school's existing coordinates are within MAX_KM of London."""
    coords = _existing_coords(row)
    if coords is None:
        return True  # no coords — try geocoding anyway
    return coords.distance_km_to(LONDON) <= MAX_KM


def _already_done(row: dict[str, Any]) -> bool:
    """True if the row already has corrected coordinates."""
    lat = row.get("CorrectedLatitude", "").strip()
    lng = row.get("CorrectedLongitude", "").strip()
