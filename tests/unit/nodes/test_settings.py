...
class TestSettingsWriteGuard:
    ...
    def test_guard_refuses_pytest_write_when_isolation_not_armed(self):
        ...
        moment of the write. When the caller claims pytest mode but the
        isolation flag is False, the guard must refuse. The isolation flag
        is injected via the DI seam, never monkeypatched."""
        from houses.nodes.settings import guard_settings_write

        with pytest.raises(RuntimeError, match="isolation fixture is not armed"):
            guard_settings_write(
                testing=True, isolation_armed=False, app_mode=False, scripts_may_write=False
            )
