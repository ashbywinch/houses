...
def test_single_cell_one_loop():
    loops = union_outline({GridCell(0, 0)}, GRID)
    assert len(loops) == 1
    assert loops[0] == [
        GeoPoint(51.0, -1.0),
        GeoPoint(51.0, -0.9),
...
def test_two_cells_one_rectangle():
    loops = union_outline({GridCell(0, 0), GridCell(0, 1)}, GRID)
    assert len(loops) == 1
    assert len(loops[0]) == 4  # the 1x2 strip's corners


def test_l_shape_outline():
    loops = union_outline({GridCell(0, 0), GridCell(0, 1), GridCell(1, 0)}, GRID)
    assert len(loops) == 1
    assert set(loops[0]) == {
        GeoPoint(51.0, -0.8),
        GeoPoint(51.1, -0.8),
...
def test_pinch_point_diagonal_cells():
    ...
    loops = union_outline({GridCell(0, 0), GridCell(1, 1)}, GRID)
    assert len(loops) == 2
    assert all(len(loop) == 4 for loop in loops)
    assert GeoPoint(51.1, -0.9) in loops[0] and GeoPoint(51.1, -0.9) in loops[1]  # the shared pinch vertex


def test_disconnected_components_two_loops():
    loops = union_outline({GridCell(0, 0), GridCell(2, 2)}, GRID)
    assert len(loops) == 2
...
def test_adjacent_cells_connect_with_irrational_grid_step():
    ...
    loops = union_outline({GridCell(10, 10), GridCell(10, 11)}, real_grid)
    assert len(loops) == 1
    assert len(loops[0]) == 4  # one closed rectangle, not fragments


def test_outline_deterministic():
    cells = {GridCell(0, 0), GridCell(0, 1), GridCell(1, 0), GridCell(1, 2)}
